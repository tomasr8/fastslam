...
def get_landmark_measurements(s, landmarks, sigma):
    x, y, _ = s

    measurements = []
    for a, b in landmarks:
        dist = math.sqrt((x - a)**2 + (y - b)**2)
        measurements.append(dist + np.random.normal(0, sigma))
...
if __name__ == "__main__":
    ...
    for i in range(15):
        ...
        s = move_vehicle_exact(s, u, dt=1)
        ss = move_vehicle_stochastic(ss, u, dt=1, sigmas=[0.1, 0.1])
        s_history.append(s)
        ss_history.append(ss)

        particles = predict(particles, u, sigmas=[0.1, 0.1], dt=1)
        # print(particles)
        weights = update(particles, weights, get_landmark_measurements(ss, landmarks, 0.2), 0.2, landmarks)
