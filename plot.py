...
with open("out_jacobian.json") as f:
    output = json.load(f)


fig, ax = plt.subplots()

ground = np.array(output["ground"])
predicted = np.array(output["predicted"])

# print(ground.shape)

for i in range(1, ground.shape[0]):
    ax.clear()
    plot_history(ax, ground[:i], color='green')
    plot_history(ax, predicted[:i], color='orange')
